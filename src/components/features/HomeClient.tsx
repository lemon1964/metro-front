...
export default function HomeClient({ headline, initialForm }: Props) {
  ...
  const dobTrackedRef = useRef(false);
  ...
  const handleDobChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // обновляем форму (как делал onField("dob"))
    setForm(s => ({ ...s, dob: next }));
  
    // трекаем ровно один раз: когда DOB впервые стал непустым
    if (!dobTrackedRef.current && next) {
      dobTrackedRef.current = true;
      track("birthday_entered", "myself-on-metro");
    }
  };
