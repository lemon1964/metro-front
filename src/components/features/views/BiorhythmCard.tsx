  ...
  return (
    <FadeCard>
      ...
      <div className="space-y-1">
        <div>Физический: <b>{result.bio.phys.toFixed(2)}</b></div>
        <div>Эмоциональный: <b>{result.bio.emo.toFixed(2)}</b></div>
        <div>Интеллектуальный: <b>{result.bio.intel.toFixed(2)}</b></div>
      </div>

      <ul className="mt-3 text-sm list-disc pl-5 space-y-1">
        {result.physHints.map((h, i) => <li key={`p${i}`}>{h}</li>)}
        {result.emoHints.map((h, i) => <li key={`e${i}`}>{h}</li>)}
        {result.intHints.map((h, i) => <li key={`i${i}`}>{h}</li>)}
      </ul>

      <div className="mt-3 text-xs text-gray-500">
        Ближайшие «критические» дни:
        {" "}физ: {result.zeros.physIn ?? "—"} дн.,
        {" "}эмо: {result.zeros.emoIn ?? "—"} дн.,
        {" "}интеллект: {result.zeros.intelIn ?? "—"} дн.
      </div>
